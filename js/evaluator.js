...
function getBinaryOp(token, level) {
...
function tokenize(line) {
    ...
    while (pos < line.length) {
        ...
        if (line[pos] == ' ') {
            ...
            while (pos1 < line.length && line[pos1].match(/[0-9\.]/))
                pos1++;
            tokens.push([line.slice(pos, pos1), 'n', pos]);
        }
        else if (line[pos].match(/[A-Za-z]/)) {
            while (pos1 < line.length && line[pos1].match(/[A-Za-z0-9]/))
                pos1++;
...
function calculate(tokens) {
    ...
    // unary operator
    if (tokens[0][1] == 'u') {
        let f = getUnaryOp(tokens[0]);
        return f(calculate(tokens.slice(1)));
    }
    // unary function
    if (tokens[0][1] == '1') {
        let f = getUnaryFunc(tokens[0]);
        if (tokens[1][0] == '(')
...
export { tokenize, classify, calculate, evaluate };
